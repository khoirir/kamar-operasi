...
class M_tindakan extends CI_Model
{
    ...
    {
        $this->db->where_in("kdPetugasMedis", array("P011","P219","P220","P059","P057","P135","P104"));
        $this->db->order_by("namapetugasMedis", "ASC");
        return $this->db->get("t_tenagamedis2");
    }
    ...
    {
        return $this->db->query("SELECT kdkelas,kelas FROM vw_tindakanoperasi GROUP BY kdkelas");
    }

    function getTindakanOP($kdkelas)
    {
        $this->db->where("kdkelas", $kdkelas);
        return $this->db->get("vw_tindakanoperasi");
    }
    ...
    function updateTindakanAnestesi($notindakananestesi, $data)
    {
        ...
        return $this->db->update('t_tindakananestesi', $data);
    }

    function insertTindakanAnestesi($data)
    {
        return $this->db->insert('t_tindakananestesi', $data);
    }
    ...
    function updateTindakanOperasi($notindakanop, $data)
    {
        ...
        return $this->db->update('t_tindakanop', $data);
    }
    ...
    {
        return $this->db->insert('t_detailtindakanop', $data);
    }

    function updateDetailTindakanOperasi($iddetail, $data)
    {
        $this->db->where('idDetailTindakan', $iddetail);
        return $this->db->update('t_detailtindakanop', $data);
    }
    ...
    function getKelasTindakanRR()
    {
        return $this->db->query("SELECT kdkelas,kelas FROM vw_tindakanrr GROUP BY kdkelas");
    }

    function getTindakanRR($kdkelas)
    {
        $this->db->where("kdkelas", $kdkelas);
        return $this->db->get("vw_tindakanrr");
    }

    function getTindakanRRPasien($notindakanop)
    {
        $select = "trr.idDetail AS iddetail,DATE_FORMAT(trr.tglTindakanRR,'%d-%m-%Y %H:%i:%s') AS tgltindakan,trr.petugas AS petugas,trr.kdTindakan AS kdtindakan,trr.tindakan AS tindakan,tk.kelas AS kelas,trr.jmlTindakan AS jmltindakan,trr.kdTarif AS kdtarif,trr.tarif AS tarif,trr.subTotal AS subtotal";
        $this->db->select($select);
    ...
    {
        $this->db->where("kdPetugasMedis IN ('P059','P054')");
        return $this->db->get("t_tenagamedis2");
    }

    function deleteDetailTindakanRR($idDetail, $idUser)
    {
        $sql = $this->db->query("UPDATE t_tindakanoprr SET statusHapus=?,idUser=CONCAT(COALESCE(idUser,''),';',?),tglUser=CONCAT(COALESCE(tglUser,''),';',NOW()) WHERE idDetail IN($idDetail)", array('1', $idUser));
        return $sql;
    }

    function updateDetailTindakanRR($tgltindakan, $petugas, $jmltindakan, $subtotal, $iduser, $iddetail)
    {
        $sql = $this->db->query("UPDATE t_tindakanoprr SET tglTindakanRR=?,petugas=?,jmlTindakan=?,subTotal=?,idUser=CONCAT(COALESCE(idUser,''),';',?),tglUser=CONCAT(COALESCE(tglUser,''),';',NOW()) WHERE idDetail=?", array($tgltindakan, $petugas, $jmltindakan, $subtotal, $iduser, $iddetail));
        return $sql;
    }

    function insertDetailTindakanRR($notindakanop, $tgltindakan, $petugas, $kdtindakan, $tindakan, $jmltindakan, $kdtarif, $tarif, $subtotal, $iduser)
    {
        $sql = $this->db->query("INSERT INTO t_tindakanoprr (noTindakanOP,tglTindakanRR,petugas,kdTindakan,tindakan,jmlTindakan,kdTarif,tarif,subTotal,statusHapus,idUser,tglUser) VALUES (?,?,?,?,?,?,?,?,?,?,CONCAT(';',?),CONCAT(';',NOW()))", array($notindakanop, $tgltindakan, $petugas, $kdtindakan, $tindakan, $jmltindakan, $kdtarif, $tarif, $subtotal, 0, $iduser));
        return $sql;
    }
    ...
    function updateDiagnosaPrapost($iddiagnosa, $data)
    {
        ...
        return $this->db->update('t_diagnosapasienop', $data);
    }
