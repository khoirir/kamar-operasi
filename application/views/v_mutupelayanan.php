            ...
            success: function (data) {
                if(data!=null){
                    ...
                    if(data.dokterop!=null){
                        ...
                        _("dokterop").value=list.substring(1);
                    }
                    _('keterangan').value=data.keterangan==""?"-":data.keterangan;
                    $("#kamarop").val(data.kamarop);

                    getDetailPasien(data.norm);
                    getDetailAsalPasien(data.noregistrasi);
                    getMutuPelayanan(data.noregistrasiop);
                    localStorage.setItem("noregistrasiop", data.noregistrasiop);
                    window.history.pushState("", "", '/kamar_operasi/mutupelayanan');
                }else{
                    getDetail(localStorage.getItem("noregistrasiop"));
                }
