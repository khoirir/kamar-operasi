...
<head>
    ...
    <title>Kamar Operasi - RSUKH</title>
    ...
    <div class="paper">
        ...
        foreach ($pasien as $p) { ?>
            <div class="kwitansi">
                <div class="title">
                    <table style="width: 100%;">
                        <tr>
                            ...
                            <td>
                                <p>KWITANSI PEMERIKSAAN KAMAR OPERASI<br>RUMAH SAKIT KARSA HUSADA BATU<br>JL. A. YANI 10 – 13 BATU<br />Telp. (0341) 596898 – 591076 Fax. (0341) 596901</p>
                            </td>
                ...
                <div class="tindakan">
                    <table style="width: 100%;">
                        ...
                        <tbody>
                            ...
                            // $tindakan = $this->db->get('t_detailtindakanop')->result();
                            $tindakan = $this->db->query("SELECT tindakan,jmlTindakan AS jml,operator,FORMAT(tarif,2,'de_DE') AS tarif,FORMAT(subTotal,2,'de_DE') AS subtotal,subTotal AS sbt FROM t_detailtindakanop WHERE noTindakanOP=? AND statusHapus=? UNION ALL SELECT tindakan,jmlTindakan AS jml,petugas AS operator,FORMAT(tarif,2,'de_DE') AS tarif,FORMAT(subTotal,2,'de_DE') AS subtotal,subTotal AS sbt FROM t_tindakanoprr WHERE noTindakanOP=? AND statusHapus=?", array($p->notinop, '0', $p->notinop, '0'))->result();
                            $total = 0.00;
                            foreach ($tindakan as $v) {
                                $operator_value = "";
                                if (substr($v->operator, 0, 1) == ";") {
                                    $get_operator = $v->operator;
                                    $arr_data = explode(";", $get_operator);
                                    if (count($arr_data) <= 2) {
                                        $operator_value = substr($get_operator, 1);
                                    } else {
                                        $arrdokter = implode("<br/>" . "- ", $arr_data);
                                        $operator_value = substr($arrdokter, 5);
                                    }
                                } else {
                                    $operator_value = $v->operator;
                                }
                                $total += $v->sbt;
                            ?>
                                <tr>
                ...
                <hr>
                <h6 style="margin-top: -5px;"><i><?php date_default_timezone_set('Asia/Jakarta');
                                                    echo '&nbsp;&nbsp;&nbsp;' . strtoupper($nama) . ', ' . date("d-m-Y H:i:s"); ?></i></h6>
